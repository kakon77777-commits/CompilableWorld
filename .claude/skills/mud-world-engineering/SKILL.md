...
**If a World Seed Package already exists** for this source (a `manifest.json` plus JSON/CSV tables — see `world_ir/SEED_FORMAT.md`) instead of raw prose, don't re-read the whole novel: `python -c "from world_ir import load_seed, validate_seed; ..."` (or `.claude/skills/mud-world-engineering/scripts/validate_seed.py <manifest.json>`) loads and structurally validates it. A Seed is Canon-tier data, not a World IR — Stage 2 still has to adapt it (paper 06 ch.9: linear plot → quest graph, ability prose → rule numbers, etc.), just from validated structured records instead of re-parsing text each time.
...
- `world_ir/SEED_FORMAT.md` (project root, not under this skill folder) — World Seed Package manifest format, for large/structured source material that doesn't fit hand-authored World IR YAML; see `docs/whitepapers/06-novel-to-mud-world-adaptation.md` for the design
