# CompilableWorld — Evennia Prototype (superseded)

**2026-07-13: this repo is now the Evennia-based prototype/reference implementation, not the main line of development.** Per `docs/whitepapers/07-evennia-to-mssp-runtime.md`, the project is pivoting to a self-built **MSSP Modular World Runtime** (World Kernel + mechanism modules + UI-decoupled projections, Evennia demoted to reference implementation / compatibility adapter). That successor project lives elsewhere (a new folder, not yet linked from here). Everything in this repo — the World IR schema/validator/compiler, the World Seed Package loader (`world_ir/seed.py`), the Chinese localization work, the real running Evennia integration — stays valid as a proven reference and a possible compatibility adapter target; none of it was wasted, it's just no longer the canonical Runtime.
...
```
CompilableWorld/
├── docs/whitepapers/       — the 7 design documents this project is built from, read in order
...
6. **`06-novel-to-mud-world-adaptation.md`** (Chinese only) — the direct answer to real friction hit adapting a real novel (命運之欲) into World IR: re-reading a whole novel every time the game needs context is expensive, slow, and drifts. Splits AI use into two stages — offline world adaptation (novel → entity/timeline/geography extraction → a versioned, provenance-tracked **World Seed Package** of JSON/CSV/EML/Manifest) and in-game local generation (only load the current region/scene/actors' slice of the seed, never the whole novel). Defines three Canon tiers (Source Canon / Adapted Canon / Generated Expansion) so narrative fact, gameplay adaptation, and runtime invention never get conflated. Structural loading/validation of this implemented in `world_ir/seed.py` — see `world_ir/SEED_FORMAT.md`.
7. **`07-evennia-to-mssp-runtime.md`** (Chinese only) — **the pivot document.** Argues Evennia's core abstractions (traditional MU*-centric) can't naturally support what the project grew into (State/Action/Event IR, hierarchical FSMs, MSSP mechanism modules, multi-UI projections, AI/Agent governance) without so much rework that forking stops saving effort. Demotes Evennia to reference implementation / prototype tool / compatibility adapter target, and proposes a self-built **MSSP Modular World Runtime**: a minimal World Kernel (entity registry, state store, action runtime, transition engine, event bus, scheduler, permission engine, persistence) with all gameplay mechanics (combat/quest/economy/etc.) as swappable TMS modules, UI fully decoupled via View Model projections, suggested stack Rust (kernel) + Python (compiler/AI/tooling). **This repo is the "before" side of that pivot** — see the README banner above.
